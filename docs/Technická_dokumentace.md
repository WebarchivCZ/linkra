...
8. Následně by mělo být možné spustit worker pomocí `node main.js run`. Je možné příkaz opakovat a spustit více instancí worker.
...
Worker čte z fronty požadavky na sklizení URL adres. Aktuálně je implementován jako nodejs script s použitím sklízeče Scoop. Tento trochu netradiční nástroj Scoop (oproti např. sklízeči Heritrix, který je obvykle webovými archivy používán) umožňuje rychlé sklizení jedné URL, což umožňuje urychlené generování archivních adres, protože nemusíme čekat na zaindexování všech dat.
